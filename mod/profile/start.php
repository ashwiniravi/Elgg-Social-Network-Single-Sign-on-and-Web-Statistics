<?php
	 ...
	 */
		function profile_page_handler($page) {
			...
			$profile_owner_guid=get_user_by_username($page[0])->guid; 
		
			if ($_SESSION['user']->getGUID() != $profile_owner_guid) {
			$visitor_guid_value=$_SESSION['user']->getGUID();
			$time_created_value=time();

			$query_result=insert_data("insert into {$CONFIG->dbprefix}profile_visitors set visitor_guid = {$visitor_guid_value}, profile_owner_guid = 				{$profile_owner_guid}, time_created = {$time_created_value}");
			} 
}
			// Any sub pages?
			if (isset($page[1])) {
